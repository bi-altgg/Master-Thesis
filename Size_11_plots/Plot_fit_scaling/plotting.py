...
def func(x,a,b):
    ...
    return x**(-a)*b
sgstrn = [0.00,0.52,1.00]
eigval = ['eigval','.','eigval','.']
fitsite = [100.0,100.0,100.0,10.0]
ax1 = plt.subplot(131)
for i in range(3):
    ...
    plt.text(60,0.7,'(a)')
    plt.legend()
sgstrn = [0.90,-0.90,0.28,1.36]
eigval = ['.','.','eigval','eigval']
fitsite = [100.0,100.0,100.0,100.0]
ax2 = plt.subplot(132)
for i in range(4):
    plot = np.loadtxt('(0.5)conductivity v_s strength_at11site'+'%1.2f'%sgstrn[i]+'.txt', dtype = 'float')
    keep_point = np.where(plot > (1.0E-18))
    plot=plot[keep_point]
    gamma_str = np.loadtxt('0.511x-axis.txt',dtype = 'float')
    gamma_str=gamma_str[keep_point]
    fittingx = np.loadtxt('0.511x-axis.txt',dtype = 'float')
    keep_point2 = np.where(fittingx > fitsite[i])
    fittingx = fittingx[keep_point2]
    fittingy = np.loadtxt('(0.5)conductivity v_s strength_at11site'+'%1.2f'%sgstrn[i]+'.txt', dtype = 'float')
    fittingy = fittingy[keep_point2]
    popt, pcov = curve_fit(func, fittingx, fittingy, maxfev=300000)
    residuals = fittingy- func(fittingx, *popt)
    ss_res = np.sum(residuals**2)
