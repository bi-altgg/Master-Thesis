...
ax1 = plt.subplot(131)
for i in range(4):
    plot = np.loadtxt('(0.5)datafile_for_11site'+ str(sgstrn[i]) + '.txt', dtype = 'float')
    keep_point = np.where(plot > 1.0E-18)
    plot=plot[keep_point]
    free_energy = np.loadtxt('free_energ(0.5).txt',dtype = 'float')
...
ax2 = plt.subplot(132)
for i in range(4):
    plot = np.loadtxt("(1.0)datafile_for11"+ str(sgstrn[i]) + ".txt", dtype = "float")
    keep_point = np.where(plot > 1.0E-18)
    plot=plot[keep_point]
...
ax3 = plt.subplot(133)
for i in range(4):
    plot = np.loadtxt("(1.5)datafile_for11"+ str(sgstrn[i]) + ".txt", dtype = "float")
    keep_point = np.where(plot > 1.0E-18)
    plot=plot[keep_point]
    free_energy = np.loadtxt("(1.5)free_energ11.txt",dtype = 'float')
    free_energy=free_energy[keep_point]
    plt.scatter(free_energy, plot,s,c = colormark[i][0],marker = colormark[i][1],alpha = .8,label =  f'$\gamma = {sgstrn[i]}$')
