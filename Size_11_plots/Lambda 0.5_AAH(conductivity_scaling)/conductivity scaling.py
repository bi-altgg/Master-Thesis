...
def trnasmission(sgindx1,sgstrn1,sgindx2,sgstrn2,energy, arraysitegamstrn):
    ...
    return np.trace(mat)
mat = []
free_energy = np.loadtxt('eigenvalue(0.5)11.dat',dtype = 'float')
...
np.savetxt('(0.5)conductivity v_s strength_at11site'+'%1.2f'%fe+'.txt',plot)
mat=[]
for sgstrn in tqdm(arrayofsitegamstrn):
    fe = free_energy[6]
    print(sgstrn,fe)
