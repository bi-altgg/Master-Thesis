...
energyval = np.loadtxt('eigenvalue(0.5)51.dat',dtype = 'float')
print(energyval)
...
def trnasmission(sgindx1,sgstrn1,sgindx2,sgstrn2,energy, arraysitegamstrn):
    ...
    return np.trace(mat)
pun = np.linspace(start=-2.0*to,stop=2.0*to,endpoint=True,num=1000)
grofer = makeeigran(energyval, 0.01, 40)
free_energy = [*pun,*grofer,*energyval]
np.savetxt('free_energ51.txt', free_energy)
for sgstrn in tqdm(arrayofsitegamstrn):
    ...
    print(plot)
    np.savetxt('(0.5)datafile_for_51site'+ str(sgstrn[2]) + '.txt', plot, )
