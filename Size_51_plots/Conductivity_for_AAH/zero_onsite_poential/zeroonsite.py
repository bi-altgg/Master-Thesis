...
arrayofsitegamstrn = [sitegammastrn0, sitegamaastrn0_5, sitegammastrn1_0,sitegamaastrn5_0]
...
print(sys_Ham)
energyval = np.loadtxt('eigenvalue(0.0)11.dat', dtype = float)
np.savetxt('Energyval.txt', energyval)
...
def trnasmission(sgindx1,sgstrn1,sgindx2,sgstrn2,energy, arraysitegamstrn):
    ...
    return np.trace(mat)
pun = np.linspace(start=-3.0*to,stop=3.0*to,endpoint=True,num=1000)
grofer = makeeigran(energyval, 0.1, 40)
free_energy = [*pun,*grofer,*energyval]
np.savetxt('(0.0)free_energ51.txt', free_energy)
for sgstrn in arrayofsitegamstrn:
    ...
    np.savetxt('(0.0)datafile_for_51lattice'+ str(sgstrn[2]) + '.txt', plot, )
