...
def func(x,a,b):
    ...
    return x**(-a)*b
sgstrn = [0.00,0.61,-0.61, 2.00]
eigval = ['eigval','.','eigval','.']
fitsite = [10.0,10.0,10.0,10.0]
for i in range(3):
    plot = np.loadtxt('(0.0)conductivity v_s strength_at_energy_small'+'%1.2f'%sgstrn[i]+'.txt', dtype = 'float')
    ...
    fittingy = np.loadtxt('(0.0)conductivity v_s strength_at_energy_small'+'%1.2f'%sgstrn[i]+'.txt', dtype = 'float')
