...
def current_vallp(mu_p,mu_l):#integral equation
    point = np.linspace(mu_l,mu_p,2000)
...
def current_valpr(mu_p,mu_r):
    point = np.linspace(mu_r,mu_p,2000)
...
def current_int(mu_p,mu_l,mu_r):
    point1 = np.linspace(mu_l,mu_p,2000)
    I1 = current_vallp(mu_p,mu_l)
    point2 = np.linspace(mu_r,mu_p,2000)
...
def current_fulllr(mu_l, mu_r):
    point = np.linspace(mu_l,mu_r,2000)    
...
def current_fulllp(mu_r, mu_l):
    ...
    point = np.linspace(mu_poi,mu_r,2000) 
...
def current_full2(mu_l,mu_r):  
    mu_poi = min_probe_potential(mu_l,mu_r)
    point1 = np.linspace(mu_l,mu_r,2000) 
    I1 = current_fulllr(mu_l,mu_r)
    point2 = np.linspace(mu_poi,mu_r,2000) 
    I2 = current_fulllp(mu_r, mu_l)
    return(integrate.simps(I1,point1) + integrate.simps(I2,point2))
current_array = []
for i in range(len(del_mu)): 
    current_array.append((current_full2(mu_L,mu_R[i]))/abs(mu_R[i]-mu_L))
    print('done'+ i)
np.savetxt("current_for_(0.5)probstrn_0.02_mul.txt",current_array)
