...
mu_R = 0.02 + 1.0e+0 #right chemical potential(first value, last value, no. of values)
print(mu_R)
...
def current_valpr(mu_p,mu_r):
    ...
    with ProcessPoolExecutor() as executor:
        result = list(executor.map(parrelel_trnas2,point))
    second_trans_probe = result
    print('k')
    return second_trans_probe
def current_int(mu_p,mu_l,mu_r):
    point1 = np.linspace(mu_l,mu_p,3)
    I1 = current_vallp(mu_p,mu_l)
    point2 = np.linspace(mu_r,mu_p,3)
